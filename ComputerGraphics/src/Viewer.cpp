...
void makeOrthogonalTo(glm::vec3 &vec1, glm::vec3 &vec2)
{
	...
	if (length == 0) {
		std::cout
			<< "Warning: called MathVec3D::makeOrthogonalTo with zero vector as the argument.  Doing nothing."
			<< std::endl;
	} else {
...
void Viewer::getFrustrumInfo()
{
	...
	m_displayHeight =
		2.0 * glm::length(m_viewCenter - m_viewPoint) * tan(Convert(0.5 * m_fieldOfView));
