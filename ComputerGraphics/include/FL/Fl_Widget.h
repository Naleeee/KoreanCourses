...
/** This struct stores all information for a text or mixed graphics label.
    ...
    \todo There is an aspiration that the Fl_Label type will become a widget by
   itself. That way we will be avoiding a lot of code duplication by handling
   labels in a similar fashion to widgets containing text. We also provide an
...
protected:
  /** Creates a widget at the given position and size.
     ...
     window \param[in] w, h size of the widget in pixels \param[in] label
  ...
  */
  enum {
    INACTIVE = 1 << 0, ///< the widget can't receive focus, and is disabled but
                       ///< potentially visible
    INVISIBLE =
        1
        << 1, ///< the widget is not drawn, but can receive a few special events
    OUTPUT = 1 << 2,   ///< for output only
    NOBORDER = 1 << 3, ///< don't draw a decoration (Fl_Window)
    FORCE_POSITION =
        1
        << 4, ///< don't let the window manager position the window (Fl_Window)
    ...
    VISIBLE_FOCUS = 1 << 9, ///< accepts keyboard focus navigation if the widget
                            ///< can have the focus
    COPIED_LABEL = 1 << 10, ///< the widget label is internally copied, its
                            ///< destruction is handled by the widget
    CLIP_CHILDREN =
        1 << 11, ///< all drawing within this widget will be clipped (Fl_Group)
    MENU_WINDOW = 1 << 12, ///< a temporary popup window, dismissed by clicking
                           ///< outside (Fl_Window)
    TOOLTIP_WINDOW = 1 << 13, ///< a temporary popup, transparent to events, and
                              ///< dismissed easily (Fl_Window)
    MODAL =
        1 << 14, ///< a window blocking input to all other winows (Fl_Window)
    NO_OVERLAY =
        1 << 15, ///< window not using a hardware overlay plane (Fl_Menu_Window)
    GROUP_RELATIVE = 1 << 16, ///< Reserved, not implemented. DO NOT USE.
    COPIED_TOOLTIP = 1 << 17, ///< the widget tooltip is internally copied, its
                              ///< destruction is handled by the widget
    FULLSCREEN = 1 << 18,     ///< a fullscreen window (Fl_Window)
    MAC_USE_ACCENTS_MENU =
        1 << 19, ///< On the Mac OS platform, pressing and holding a key on the
                 ///< keyboard opens an accented-character menu window
...
public:
  /** Destroys the widget.
     ...
     widgets and groups in that group.

      \since FLTK 1.3, the widget's destructor removes the widget from its
     parent group, if it is member of a group.
   */
  virtual ~Fl_Widget();
  ...
  */
  void parent(Fl_Group *p) {
    parent_ = p;
  } // for hacks only, use Fl_Group::add()
  ...
  /** Gets the user data for this widget.
     ...
     callback function. \return user data as a pointer
   */
  void *user_data() const { return user_data_; }

  /** Sets the user data for this widget.
      Sets the new user data (void *) argument that is passed to the callback
     function. \param[in] v new user data
   */
  void user_data(void *v) { user_data_ = v; }

  /** Gets the current user data (long) argument that is passed to the callback
     function. \todo The user data value must be implemented using \em intptr_t
     or similar to avoid 64-bit machine incompatibilities.
   */
  long argument() const { return (long)(fl_intptr_t)user_data_; }

  /** Sets the current user data (long) argument that is passed to the callback
     function. \todo The user data value must be implemented using \em intptr_t
     or similar to avoid 64-bit machine incompatibilities.
   */
  ...
  /** Marks the widget as inactive without sending events or changing focus.
      This is mainly for specialized use, for normal cases you want
     deactivate(). \see deactivate()
   */
