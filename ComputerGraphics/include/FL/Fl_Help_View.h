...
struct FL_EXPORT Fl_Help_Font_Stack {
  ...
  void push(Fl_Font f, Fl_Fontsize s, Fl_Color c) {
    if (nfonts_ < MAX_FL_HELP_FS_ELTS - 1)
      nfonts_++;
    elts_[nfonts_].set(f, s, c);
    fl_font(f, s);
    fl_color(c);
  ...
  size_t count() const {
    return nfonts_;
  } // Gets the current number of fonts in the stack
...
/**
  ...
  Quoted char names:
     - Aacute aacute Acirc acirc acute AElig aelig Agrave agrave amp Aring aring
  Atilde atilde Auml auml
     ...
     - Oacute oacute Ocirc ocirc Ograve ograve ordf ordm Oslash oslash Otilde
  otilde Ouml ouml
...
class FL_EXPORT Fl_Help_View : public Fl_Group { // Help viewer widget
  ...
  Fl_Help_Block *add_block(const char *s, int xx, int yy, int ww, int hh,
                           uchar border = 0);
  void add_link(const char *n, int xx, int yy, int ww, int hh);
  void add_target(const char *n, int yy);
  static int compare_targets(const Fl_Help_Target *t0,
...
public:
  ...
  */
  void scrollbar_size(int newSize) { scrollbar_size_ = newSize; }
};
